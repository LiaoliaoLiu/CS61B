...
public class Model extends Observable {
    ...
    private boolean tileWithoutMerge() {
        ...
        for (int c = 0; c < this.board.size(); ++c) {
            ...
            Tile currentTile;
            for (int r = boundary; r >= 0; --r) {
                currentTile = this.board.tile(c, r);
                if (currentTile != null) {
                    this.board.move(c, boundary, currentTile);
                    if (boundary != r) {
                        moved = true;
                    }
                    --boundary;
    ...
    private boolean merge() {
        ...
        for (int c = 0; c <= boundary; ++c) {
            int mergeBoundary = boundary; // indicate the toppest row that merge can happen
            for (int r = boundary - 1; r >= 0; --r) {

                checkTile = this.board.tile(c, r);
                adjacentTile = this.board.tile(c, r + 1);
                if (r + 1 <= mergeBoundary && mergeable(checkTile, adjacentTile)) {
                    this.board.move(c, r + 1, checkTile);
                    mergeBoundary -= 2;
    ...
    private static boolean mergeable(Board b) {
        ...
        for (int c = 0; c < boundary; ++c) {
            for (int r = 0; r < boundary - 1; ++r) {
                ...
                if (mergeable(checkTile, adjacentTile)) {
                    return true;
                }
            }
        }
    ...
    private static boolean mergeable(Tile checkTile, Tile adjacentTile) {
